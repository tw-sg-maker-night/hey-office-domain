...
# - DOMAIN
# - API_GATEWAY_NAME
# - DOMAIN_BASE_PATH_MAPPING (defaults to '(none)'. Change when there is a static suffix after your domain, eg. 'api.example.com/v1/*', DOMAIN_BASE_PATH_MAPPING=v1 )
...
###############################################################################
# Validate the environment is setup correctly
...
os.environ['DOMAIN_BASE_PATH_MAPPING'] = os.environ.get('DOMAIN_BASE_PATH_MAPPING', '(none)')
...
def main():
    ...
    base_path_mapping_cmd = [
        ...
        '--domain-name', cust_env['DOMAIN'],
        '--base-path', cust_env['DOMAIN_BASE_PATH_MAPPING'],
        '--query', 'restApiId'
    ]
    ...
    try:
        ...
        create_base_path_mapping_cmd = [
            'aws', 'apigateway', 'create-base-path-mapping',
            '--domain-name', cust_env['DOMAIN'],
            '--rest-api-id', api_gateway_id
        ]
        if cust_env['DOMAIN_BASE_PATH_MAPPING'] is not '(none)':
            create_base_path_mapping_cmd.append('--base-path')
            create_base_path_mapping_cmd.append(cust_env['DOMAIN_BASE_PATH_MAPPING'])

        subprocess.call(create_base_path_mapping_cmd)
